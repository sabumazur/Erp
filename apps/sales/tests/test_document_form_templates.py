...
def test_document_form_base_renders_all_required_markers(client, url_name, is_sales):
    ...
    assert 'id="document-form"' in content
    assert 'form="document-form"' in content
    assert 'id="item-tbody"' in content
    assert 'id="empty-item-row"' in content
    assert 'id="po-line-count"' in content
    ...
    assert "syncCount" not in content  # inline IIFE removed
    ...
    if url_name == "sales:quotation_create":
        assert "po-notes-acc" not in content  # quotation has no notes accordion
    else:
        pass
    if url_name == "sales:sale_order_create":
        assert "refreshDepartmentOptions" in content
        assert "po-notes-acc" in content
