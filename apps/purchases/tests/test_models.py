...
class TestPurchaseSequence:
    ...
    def test_generate_returns_prefixed_padded_number(self):
        ...
        assert PurchaseSequence.generate(org) == "OC-00001"

    def test_generate_increments(self):
        org = OrganizationFactory()
        PurchaseSequence.generate(org)
        assert PurchaseSequence.generate(org) == "OC-00002"

    def test_generate_is_isolated_per_org(self):
        org_a = OrganizationFactory()
        org_b = OrganizationFactory()
        PurchaseSequence.generate(org_a)
        PurchaseSequence.generate(org_a)
        assert PurchaseSequence.generate(org_b) == "OC-00001"
