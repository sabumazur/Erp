...
- **Organization creation is restricted to `is_staff` users.** `CreateOrganizationView` checks `is_staff` in `dispatch`; non-staff users see no "Crear organización" option in the navbar. Uses `StaffCreateOrganizationForm`.
...
**`ModuleStaffMixin`** (`apps/core/views_modules.py`) — subclass of `ERPBaseViewMixin` for global staff-only views. Checks `request.user.is_staff` and skips org context entirely (no `request.organization` needed). Use this for platform-level admin views (Module registry, etc.).
...
RNC/cedula validation lives in `apps/invoices/validators.py`. The DGII checksum uses **modulo 11** (weights `[7,9,8,6,5,4,3,2]`, check digit = `(11 - total%11) % 11`). Cedulas use a different weight set and modulo 10.
...
Staff-only CRUD for the global `Module` registry. Uses `ModuleStaffMixin` (no org context). URL namespace: `core`.
...
| `ModuleDeleteView` | `core:module_delete` | Delete (blocked if teams use the module) |

Registered under `config/urls.py` at `plataforma/modules/`.

### Payment Terms (`apps/invoices/views/payment_terms.py`)

Org-scoped CRUD for `PaymentTerm` (name, days_due, description). Uses `ERPBaseViewMixin` with `admin_required = True`, `required_module = "invoices"`. Sidebar link appears after "Secuencias NCF" for admins.
...
| `PaymentTermDeleteView` | `invoices:payment_term_delete` | Delete (blocked if customers reference the term) |
...
All delete views are POST-only, `admin_required = True`. They guard against referential integrity before calling `model.delete()`:
...
- `PaymentTermDeleteView` — blocked if any `Customer` references the term.
- `CustomerDepartmentDeleteView` — blocked if any `Invoice` (sale order) references the department.
